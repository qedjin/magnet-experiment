...
// 모바일/저사양 환경 감지 & 경량 모드
// =========================================================
// iOS Safari는 탭당 메모리 한도가 낮아서, 그림자/안티앨리어싱/높은 픽셀비율을
// 그대로 두면 메인 메뉴에서도 reload 되는 문제가 있습니다.
// 모바일 환경에서는 시각 품질을 약간 낮춰 메모리/GPU 사용량을 줄입니다.
const __isMobile = (() => {
  if (typeof navigator === 'undefined') return false;
  // iPad 13+가 desktop UA를 쓰는 경우까지 잡으려고 touch + 좁은 화면도 함께 체크
  const ua = navigator.userAgent || '';
  const uaMobile = /iPhone|iPad|iPod|Android|Mobi/i.test(ua);
  const touchSmall = (navigator.maxTouchPoints || 0) > 0 && Math.min(window.innerWidth, window.innerHeight) < 900;
  return uaMobile || touchSmall;
})();
console.log('[mobile mode]', __isMobile);

// 3D 장면 준비 여부 플래그 (모바일에선 메뉴 클릭 후 lazy init 됨)
let __sceneStarted = false;

// 1-3. 렌더러 (그림을 그려주는 엔진)
const renderer = new THREE.WebGLRenderer({
  antialias: !__isMobile,           // 모바일에선 끔 (메모리/GPU 절약)
  powerPreference: 'high-performance',
});
const initialRenderSize = getFrameSize();
renderer.setSize(initialRenderSize.width, initialRenderSize.height, false);
// 픽셀 비율: 데스크톱은 최대 2, 모바일은 1로 고정 (4배 적은 픽셀 처리)
renderer.setPixelRatio(__isMobile ? 1 : Math.min(window.devicePixelRatio || 1, 2));
// 그림자: 모바일은 끔. 그림자 맵은 메모리와 fillrate 모두 비싼 작업.
renderer.shadowMap.enabled = !__isMobile;
...
dirLight.castShadow = !__isMobile;        // 모바일은 그림자 안 던짐
if (!__isMobile) {
...
function enableShadow(obj) {
  if (__isMobile) return;   // 모바일: 그림자 비활성 (메모리/GPU 절약)
  obj.traverse(child => {
    if (child.isMesh) {
 ...
 * - (모바일 메모리 절약) 첫 클릭 시점에 buildScene 시작 — 메인 메뉴 표시 중엔
 *   GLB를 절대 받지 않도록 한다.
 */
async function startFromMenu(mode) {
  ...
  // 3D 장면이 아직 만들어지지 않았으면 지금 만든다 (lazy init)
  // 메인 메뉴 단계에선 GLB를 일체 받지 않으므로 모바일 메모리 압박이 사라진다.
  if (!__sceneStarted) {
    __sceneStarted = true;
    const bar = document.getElementById('status-bar');
    if (bar) bar.textContent = '실험실을 준비하는 중입니다... 잠시만 기다려 주세요';
    try {
      await buildScene();
      animate();
    } catch (e) {
      console.error('buildScene 실패:', e);
      if (bar) {
        bar.style.background = '#ffebee';
        bar.style.color = '#c62828';
        bar.textContent = '실험실 준비 중 오류: ' + (e?.message || e);
      }
      return;
    }
  }
...
// 3D 장면 준비 — 메모리 절약을 위해 모바일에선 메뉴 클릭 후로 미룸
// (위 startFromMenu 안에서 __sceneStarted 플래그로 제어)
if (!__isMobile) {
  // 데스크톱: 페이지 로드 즉시 장면 준비 (사용자 경험을 위해 미리 로딩)
  __sceneStarted = true;
  buildScene().then(() => {
    animate();
  });
}
// 모바일은 startFromMenu 가 처음 호출될 때 buildScene 시작
